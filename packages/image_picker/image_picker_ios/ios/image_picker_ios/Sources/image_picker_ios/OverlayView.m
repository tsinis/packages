...
static CGFloat clamp(CGFloat v, CGFloat min, CGFloat max) { return MIN(MAX(v, min), max); }
...
- (id)initWithFrame:(CGRect)frame andPath:(NSString *)path andOpacity:(NSNumber *)opacity {
  if (self = [super initWithFrame:frame]) {
    ...
    // don't have an access to source code of default camera controls UI it can be only hardcoded
    // this way:
    ...
    CGRect previewFrame = (isLandscape && !isNewestIos)
                              ? CGRectMake(paddedOffset, 0, shortestSide, longestSide)
    ...
    // [CUIImagePickerController] preferred orientation.
    if (isLandscape && !isNewestIos) {
      // This modifier is needed for transforming landscape mode when it's in
      // [UIInterfaceOrientationLandscapeRight]
      int directionModifier =
          ([UIDevice currentDevice].orientation == UIInterfaceOrientationLandscapeLeft) ? 1 : -1;
      ...
      if (directionModifier == 1) {
        // This 1.5 is more like magic number, needs more different older devices to confirm/change
        // it.
        overlayView.transform =
            CGAffineTransformTranslate(overlayView.transform, 0, (paddedOffset * 1.5));
      }
    } else {
      // Set overlay frame to be exactly as big as camera preview
